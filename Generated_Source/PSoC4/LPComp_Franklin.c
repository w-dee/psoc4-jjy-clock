...
uint8 LPComp_Franklin_initVar = 0u;
static uint32 LPComp_Franklin_internalIntr = 0u;
    ...
    */
    CY_NOINIT static uint32 LPComp_Franklin_intrMaskRegState;
#endif /* (LPComp_Franklin_CY_LPCOMP_V2) */


/*******************************************************************************
* Function Name: LPComp_Franklin_Start
********************************************************************************
*
* Summary:
...
*  the comparator following a LPComp_Franklin_Stop() call, the current
...
*
* Global variables:
*  LPComp_Franklin_initVar: Is modified when this function is called for the
*   first time. Is used to ensure that initialization happens only once.
*
*******************************************************************************/
void LPComp_Franklin_Start(void)
{
    if(0u == LPComp_Franklin_initVar)
    {
        LPComp_Franklin_Init();
        LPComp_Franklin_initVar = 1u;
    }
    LPComp_Franklin_Enable();
}


/*******************************************************************************
* Function Name: LPComp_Franklin_Init
********************************************************************************
*
* Summary:
*  Initializes or restores the component according to the customizer settings.
*  It is not necessary to call LPComp_Franklin_Init() because the
*  LPComp_Franklin_Start() API calls this function and is the preferred method
...
void LPComp_Franklin_Init(void)
{
    ...
    LPComp_Franklin_CONFIG_REG |= LPComp_Franklin_CONFIG_REG_DEFAULT;
        ...
        if (CySysPmGetResetReason() == CY_PM_RESET_REASON_WAKEUP_HIB)
        {
            ...
            */
            LPComp_Franklin_INTR_MASK_REG |= LPComp_Franklin_intrMaskRegState;  
        }
        else
        {
            LPComp_Franklin_INTR_MASK_REG |= LPComp_Franklin_INTR_MASK_REG_DEFAULT;    
            LPComp_Franklin_intrMaskRegState = LPComp_Franklin_INTR_MASK_REG;
        }
    #endif /* (LPComp_Franklin_CY_LPCOMP_V2) */
...
/*******************************************************************************
* Function Name: LPComp_Franklin_Enable
********************************************************************************
*
* Summary:
*  Activates the hardware and begins component operation. It is not necessary
*  to call LPComp_Franklin_Enable() because the LPComp_Franklin_Start() API
...
void LPComp_Franklin_Enable(void)
{
    ...
    enableInterrupts = CyEnterCriticalSection();
    LPComp_Franklin_CONFIG_REG |= LPComp_Franklin_CONFIG_EN;
    ...
    *******************************************************/
    LPComp_Franklin_CONFIG_REG |= LPComp_Franklin_internalIntr;
    CyExitCriticalSection(enableInterrupts);
}


/*******************************************************************************
* Function Name: LPComp_Franklin_Stop
********************************************************************************
*
* Summary:
...
void LPComp_Franklin_Stop(void)
{
    ...
    enableInterrupts = CyEnterCriticalSection();
        ...
        * is cleared.
        *******************************************************/
        LPComp_Franklin_CONFIG_REG &= (uint32)~LPComp_Franklin_CONFIG_INTR_MASK;
    #endif /* (LPComp_Franklin_CY_LPCOMP_V0) */

    LPComp_Franklin_CONFIG_REG &= (uint32)~LPComp_Franklin_CONFIG_EN;
    CyExitCriticalSection(enableInterrupts);
}


/*******************************************************************************
* Function Name: LPComp_Franklin_SetSpeed
*
* Summary:
...
*   LPComp_Franklin_LOW_SPEED  - Slow/ultra low 
*   LPComp_Franklin_MED_SPEED  - Medium/low 
*   LPComp_Franklin_HIGH_SPEED - Fast/normal 
*
* Return:
*  None
*
...
void LPComp_Franklin_SetSpeed(uint32 speed)
{
    ...
    enableInterrupts = CyEnterCriticalSection();
    config = LPComp_Franklin_CONFIG_REG & (uint32)~LPComp_Franklin_CONFIG_SPEED_MODE_MASK;
    LPComp_Franklin_CONFIG_REG = config | LPComp_Franklin_GET_CONFIG_SPEED_MODE(speed);
    CyExitCriticalSection(enableInterrupts);
}


/*******************************************************************************
* Function Name: LPComp_Franklin_SetInterruptMode
********************************************************************************
*
* Summary:
...
*  LPComp_Franklin_INTR_DISABLE - Disable
*  LPComp_Franklin_INTR_RISING  - Rising edge detect
*  LPComp_Franklin_INTR_FALLING - Falling edge detect
*  LPComp_Franklin_INTR_BOTH    - Detect both edges
*
* Return:
*  None
*
...
void LPComp_Franklin_SetInterruptMode(uint32 mode)
{
    ...
    LPComp_Franklin_internalIntr = LPComp_Franklin_GET_CONFIG_INTR(mode);

    enableInterrupts = CyEnterCriticalSection();
    config = LPComp_Franklin_CONFIG_REG & (uint32)~LPComp_Franklin_CONFIG_INTR_MASK;
    LPComp_Franklin_CONFIG_REG = config | LPComp_Franklin_internalIntr;
    CyExitCriticalSection(enableInterrupts);
}


/*******************************************************************************
* Function Name: LPComp_Franklin_GetInterruptSource
********************************************************************************
*
* Summary:
...
*  LPComp_Franklin_INTR.
*
*******************************************************************************/
uint32 LPComp_Franklin_GetInterruptSource(void)
{
    return (LPComp_Franklin_INTR_REG); 
}


/*******************************************************************************
* Function Name: LPComp_Franklin_ClearInterrupt
********************************************************************************
*
* Summary:
...
*  has a mask value: LPComp_Franklin_INTR.
*
* Return:
*  None
*
...
{   
    LPComp_Franklin_INTR_REG = interruptMask;
}


/*******************************************************************************
* Function Name: LPComp_Franklin_SetInterrupt
********************************************************************************
*
* Summary:
...
*  a mask value: LPComp_Franklin_INTR_SET.
*
* Return:
*  None
*
...
void LPComp_Franklin_SetInterrupt(uint32 interruptMask)
{
    ...
    enableInterrupts = CyEnterCriticalSection();
    LPComp_Franklin_INTR_SET_REG |= interruptMask;
    CyExitCriticalSection(enableInterrupts);
}


/*******************************************************************************
* Function Name: LPComp_Franklin_SetHysteresis
********************************************************************************
*
* Summary:
...
*  LPComp_Franklin_HYST_ENABLE     - Enable hysteresis
*  LPComp_Franklin_HYST_DISABLE    - Disable hysteresis
*
* Return:
*  None
*
...
void LPComp_Franklin_SetHysteresis(uint32 hysteresis)
{
    ...
    if(0u != hysteresis)
    {
        LPComp_Franklin_CONFIG_REG |= LPComp_Franklin_CONFIG_HYST;
    }
    else
    {
        LPComp_Franklin_CONFIG_REG &= (uint32)~LPComp_Franklin_CONFIG_HYST;
    }
    CyExitCriticalSection(enableInterrupts);
}
...
/*******************************************************************************
* Function Name: LPComp_Franklin_SetFilter
********************************************************************************
*
* Summary:
...
*  LPComp_Franklin_FILT_ENABLE  - Enable filter
*  LPComp_Franklin_FILT_DISABLE - Disable filter
*
* Return:
*  None
*
...
void LPComp_Franklin_SetFilter(uint32 filter)
{
    ...
    if( 0u != filter)
    {
        LPComp_Franklin_CONFIG_REG |= LPComp_Franklin_CONFIG_FILT;
    }
    else
    {
        LPComp_Franklin_CONFIG_REG &= (uint32)~LPComp_Franklin_CONFIG_FILT;
    }
    CyExitCriticalSection(enableInterrupts);
}
...
/*******************************************************************************
* Function Name: LPComp_Franklin_GetCompare
********************************************************************************
*
* Summary:
...
{
    return((uint32)((0u == (LPComp_Franklin_CONFIG_REG & LPComp_Franklin_CONFIG_OUT)) ? 0u : 1u));
}


/*******************************************************************************
* Function Name: LPComp_Franklin_ZeroCal
********************************************************************************
*
* Summary:
...
*  LPComp_Franklin_LoadTrim() API routine.
*
*******************************************************************************/
uint32 LPComp_Franklin_ZeroCal(void)
{
    ...
    LPComp_Franklin_DFT_REG |= LPComp_Franklin_DFT_CAL_EN;
    LPComp_Franklin_TRIMA_REG = 0u;
    LPComp_Franklin_TRIMB_REG = 0u;

    CyDelayUs(1u);

    cmpOut = LPComp_Franklin_GetCompare();

    if(0u != cmpOut)
    {
        LPComp_Franklin_TRIMA_REG = ((uint32) 0x01u << LPComp_Franklin_TRIMA_SIGNBIT);
    }

    for(i = (LPComp_Franklin_TRIMA_MAX_VALUE +1u ); i != 0u; i--)
    {
        LPComp_Franklin_TRIMA_REG++;
        CyDelayUs(1u);
        if(cmpOut != LPComp_Franklin_GetCompare())
        {
            break;
        }
    }

    if(((uint32)(LPComp_Franklin_CONFIG_REG >> LPComp_Franklin_CONFIG_REG_SHIFT) &
        LPComp_Franklin_SPEED_PARAM_MASK) == LPComp_Franklin_MED_SPEED)
    {
        cmpOut = LPComp_Franklin_GetCompare();

        if(0u == cmpOut)
        {
            LPComp_Franklin_TRIMB_REG = ((uint32)1u << LPComp_Franklin_TRIMB_SIGNBIT);
        }

        for(i = (LPComp_Franklin_TRIMB_MAX_VALUE +1u ); i != 0u; i--)
        {
            LPComp_Franklin_TRIMB_REG++;
            CyDelayUs(1u);
            if(cmpOut != LPComp_Franklin_GetCompare())
            {
                break;
    ...
    LPComp_Franklin_DFT_REG &= ((uint32)~LPComp_Franklin_DFT_CAL_EN);

    return (LPComp_Franklin_TRIMA_REG | ((uint32)LPComp_Franklin_TRIMB_REG << LPComp_Franklin_TRIMB_SHIFT));
}


/*******************************************************************************
* Function Name: LPComp_Franklin_LoadTrim
********************************************************************************
*
* Summary:
...
*  LPComp_Franklin_ZeroCal() API routine.
*
* Return:
*  None
*
...
{
    LPComp_Franklin_TRIMA_REG = (trimVal & LPComp_Franklin_TRIMA_MASK);
    LPComp_Franklin_TRIMB_REG = ((trimVal >> LPComp_Franklin_TRIMB_SHIFT) & LPComp_Franklin_TRIMB_MASK);
}
...
/*******************************************************************************
* Function Name: LPComp_Franklin_SetOutputMode
********************************************************************************
*
* Summary:
...
*  LPComp_Franklin_OUT_DIRECT - Direct output
*  LPComp_Franklin_OUT_SYNC   - Synchronized output
*  LPComp_Franklin_OUT_PULSE  - Pulse output
*
* Return:
*  None
*
...
void LPComp_Franklin_SetOutputMode(uint32 mode)
{
    ...
    enableInterrupts = CyEnterCriticalSection();
    config = LPComp_Franklin_CONFIG_REG & ((uint32)~(LPComp_Franklin_CONFIG_DSI_BYPASS | \
                                                     LPComp_Franklin_CONFIG_DSI_LEVEL));
    LPComp_Franklin_CONFIG_REG = config | LPComp_Franklin_GET_CONFIG_DSI_BYPASS(mode) | \
                                           LPComp_Franklin_GET_CONFIG_DSI_LEVEL(mode);
    CyExitCriticalSection(enableInterrupts);
}


/*******************************************************************************
* Function Name: LPComp_Franklin_SetInterruptMask
********************************************************************************
*
* Summary:
...
* Parameters:
*  uint32 interruptMask: Mask of interrupts to set. Each component instance has
*  a mask value: LPComp_Franklin_INTR_MASK.
*
* Return:
*  None
*
...
{
    LPComp_Franklin_intrMaskRegState = interruptMask;
    LPComp_Franklin_INTR_MASK_REG = LPComp_Franklin_intrMaskRegState;
}


/*******************************************************************************
* Function Name: LPComp_Franklin_GetInterruptMask
********************************************************************************
*
* Summary:
...
*  has a mask value: LPComp_Franklin_INTR_MASK.
*
*******************************************************************************/
uint32 LPComp_Franklin_GetInterruptMask(void)
{
    return (LPComp_Franklin_INTR_MASK_REG);
}


/*******************************************************************************
* Function Name: LPComp_Franklin_GetInterruptSourceMasked
********************************************************************************
*
* Summary:
...
*  has a mask value: LPComp_Franklin_INTR_MASKED.
*
*******************************************************************************/
uint32 LPComp_Franklin_GetInterruptSourceMasked(void)
{
    return (LPComp_Franklin_INTR_MASKED_REG);
}
