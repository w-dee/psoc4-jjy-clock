...
uint8 LPComp_1_initVar = 0u;
static uint32 LPComp_1_internalIntr = 0u;
    ...
    */
    CY_NOINIT static uint32 LPComp_1_intrMaskRegState;
...
void LPComp_1_Init(void)
{
    ...
    LPComp_1_CONFIG_REG |= LPComp_1_CONFIG_REG_DEFAULT;
        ...
        if (CySysPmGetResetReason() == CY_PM_RESET_REASON_WAKEUP_HIB)
        {
            ...
            */
            LPComp_1_INTR_MASK_REG |= LPComp_1_intrMaskRegState;  
        }
        else
        {
            LPComp_1_INTR_MASK_REG |= LPComp_1_INTR_MASK_REG_DEFAULT;    
            LPComp_1_intrMaskRegState = LPComp_1_INTR_MASK_REG;
        }
    #endif /* (LPComp_1_CY_LPCOMP_V2) */
...
void LPComp_1_Enable(void)
{
    ...
    enableInterrupts = CyEnterCriticalSection();
    LPComp_1_CONFIG_REG |= LPComp_1_CONFIG_EN;
    ...
    *******************************************************/
    LPComp_1_CONFIG_REG |= LPComp_1_internalIntr;
    CyExitCriticalSection(enableInterrupts);
}
...
void LPComp_1_Stop(void)
{
    ...
    enableInterrupts = CyEnterCriticalSection();
        ...
        * is cleared.
        *******************************************************/
        LPComp_1_CONFIG_REG &= (uint32)~LPComp_1_CONFIG_INTR_MASK;
    #endif /* (LPComp_1_CY_LPCOMP_V0) */

    LPComp_1_CONFIG_REG &= (uint32)~LPComp_1_CONFIG_EN;
    CyExitCriticalSection(enableInterrupts);
}


/*******************************************************************************
* Function Name: LPComp_1_SetSpeed
*
* Summary:
...
*   LPComp_1_LOW_SPEED  - Slow/ultra low 
*   LPComp_1_MED_SPEED  - Medium/low 
*   LPComp_1_HIGH_SPEED - Fast/normal 
*
* Return:
*  None
*
...
void LPComp_1_SetSpeed(uint32 speed)
{
    ...
    enableInterrupts = CyEnterCriticalSection();
    config = LPComp_1_CONFIG_REG & (uint32)~LPComp_1_CONFIG_SPEED_MODE_MASK;
    LPComp_1_CONFIG_REG = config | LPComp_1_GET_CONFIG_SPEED_MODE(speed);
    CyExitCriticalSection(enableInterrupts);
}


/*******************************************************************************
* Function Name: LPComp_1_SetInterruptMode
********************************************************************************
*
* Summary:
...
*  LPComp_1_INTR_DISABLE - Disable
*  LPComp_1_INTR_RISING  - Rising edge detect
*  LPComp_1_INTR_FALLING - Falling edge detect
*  LPComp_1_INTR_BOTH    - Detect both edges
*
* Return:
*  None
*
...
void LPComp_1_SetInterruptMode(uint32 mode)
{
    ...
    LPComp_1_internalIntr = LPComp_1_GET_CONFIG_INTR(mode);

    enableInterrupts = CyEnterCriticalSection();
    config = LPComp_1_CONFIG_REG & (uint32)~LPComp_1_CONFIG_INTR_MASK;
    LPComp_1_CONFIG_REG = config | LPComp_1_internalIntr;
    CyExitCriticalSection(enableInterrupts);
}


/*******************************************************************************
* Function Name: LPComp_1_GetInterruptSource
********************************************************************************
*
* Summary:
...
*  has a mask value: LPComp_1_INTR.
*
* Return:
*  None
*
...
{   
    LPComp_1_INTR_REG = interruptMask;
}
...
*  a mask value: LPComp_1_INTR_SET.
*
* Return:
*  None
*
...
void LPComp_1_SetInterrupt(uint32 interruptMask)
{
    ...
    enableInterrupts = CyEnterCriticalSection();
    LPComp_1_INTR_SET_REG |= interruptMask;
    CyExitCriticalSection(enableInterrupts);
}
...
*  LPComp_1_HYST_ENABLE     - Enable hysteresis
*  LPComp_1_HYST_DISABLE    - Disable hysteresis
*
* Return:
*  None
*
...
void LPComp_1_SetHysteresis(uint32 hysteresis)
{
    ...
    if(0u != hysteresis)
    {
        LPComp_1_CONFIG_REG |= LPComp_1_CONFIG_HYST;
    }
    else
    {
        LPComp_1_CONFIG_REG &= (uint32)~LPComp_1_CONFIG_HYST;
    }
    CyExitCriticalSection(enableInterrupts);
}
...
/*******************************************************************************
* Function Name: LPComp_1_SetFilter
********************************************************************************
*
* Summary:
...
*  LPComp_1_FILT_ENABLE  - Enable filter
*  LPComp_1_FILT_DISABLE - Disable filter
*
* Return:
*  None
*
...
void LPComp_1_SetFilter(uint32 filter)
{
    ...
    if( 0u != filter)
    {
        LPComp_1_CONFIG_REG |= LPComp_1_CONFIG_FILT;
    }
    else
    {
        LPComp_1_CONFIG_REG &= (uint32)~LPComp_1_CONFIG_FILT;
    }
    CyExitCriticalSection(enableInterrupts);
}
...
{
    return((uint32)((0u == (LPComp_1_CONFIG_REG & LPComp_1_CONFIG_OUT)) ? 0u : 1u));
}


/*******************************************************************************
* Function Name: LPComp_1_ZeroCal
********************************************************************************
*
* Summary:
...
uint32 LPComp_1_ZeroCal(void)
{
    ...
    LPComp_1_DFT_REG |= LPComp_1_DFT_CAL_EN;
    LPComp_1_TRIMA_REG = 0u;
    LPComp_1_TRIMB_REG = 0u;

    CyDelayUs(1u);

    cmpOut = LPComp_1_GetCompare();

    if(0u != cmpOut)
    {
        LPComp_1_TRIMA_REG = ((uint32) 0x01u << LPComp_1_TRIMA_SIGNBIT);
    }

    for(i = (LPComp_1_TRIMA_MAX_VALUE +1u ); i != 0u; i--)
    {
        LPComp_1_TRIMA_REG++;
        CyDelayUs(1u);
        if(cmpOut != LPComp_1_GetCompare())
        {
            break;
        }
    }

    if(((uint32)(LPComp_1_CONFIG_REG >> LPComp_1_CONFIG_REG_SHIFT) &
        LPComp_1_SPEED_PARAM_MASK) == LPComp_1_MED_SPEED)
    {
        cmpOut = LPComp_1_GetCompare();

        if(0u == cmpOut)
        {
            LPComp_1_TRIMB_REG = ((uint32)1u << LPComp_1_TRIMB_SIGNBIT);
        }

        for(i = (LPComp_1_TRIMB_MAX_VALUE +1u ); i != 0u; i--)
        {
            LPComp_1_TRIMB_REG++;
            CyDelayUs(1u);
            if(cmpOut != LPComp_1_GetCompare())
    ...
    LPComp_1_DFT_REG &= ((uint32)~LPComp_1_DFT_CAL_EN);

    return (LPComp_1_TRIMA_REG | ((uint32)LPComp_1_TRIMB_REG << LPComp_1_TRIMB_SHIFT));
}


/*******************************************************************************
* Function Name: LPComp_1_LoadTrim
********************************************************************************
*
* Summary:
...
{
    LPComp_1_TRIMA_REG = (trimVal & LPComp_1_TRIMA_MASK);
    LPComp_1_TRIMB_REG = ((trimVal >> LPComp_1_TRIMB_SHIFT) & LPComp_1_TRIMB_MASK);
}
...
*  LPComp_1_OUT_DIRECT - Direct output
*  LPComp_1_OUT_SYNC   - Synchronized output
*  LPComp_1_OUT_PULSE  - Pulse output
*
* Return:
*  None
*
...
void LPComp_1_SetOutputMode(uint32 mode)
{
    ...
    enableInterrupts = CyEnterCriticalSection();
    config = LPComp_1_CONFIG_REG & ((uint32)~(LPComp_1_CONFIG_DSI_BYPASS | \
                                                     LPComp_1_CONFIG_DSI_LEVEL));
    LPComp_1_CONFIG_REG = config | LPComp_1_GET_CONFIG_DSI_BYPASS(mode) | \
                                           LPComp_1_GET_CONFIG_DSI_LEVEL(mode);
    CyExitCriticalSection(enableInterrupts);
}
...
* Parameters:
*  uint32 interruptMask: Mask of interrupts to set. Each component instance has
*  a mask value: LPComp_1_INTR_MASK.
*
* Return:
*  None
*
...
{
    LPComp_1_intrMaskRegState = interruptMask;
    LPComp_1_INTR_MASK_REG = LPComp_1_intrMaskRegState;
}
...
*  has a mask value: LPComp_1_INTR_MASK.
*
*******************************************************************************/
uint32 LPComp_1_GetInterruptMask(void)
{
    return (LPComp_1_INTR_MASK_REG);
}


/*******************************************************************************
* Function Name: LPComp_1_GetInterruptSourceMasked
********************************************************************************
*
* Summary:
...
*  has a mask value: LPComp_1_INTR_MASKED.
*
*******************************************************************************/
uint32 LPComp_1_GetInterruptSourceMasked(void)
{
    return (LPComp_1_INTR_MASKED_REG);
}
